...
const vector = [];
numero.toString();
numero.toFixed(2); //Redondea el numero a 2 decimales.
numero.toExponential(2); //Convierte el numero a notacion cientifica.
numero.toPrecision(2); //Convierte el numero a notacion cientifica con 2 decimales.
numero.valueOf(); //Convierte el numero a un valor numerico.
typeof numero; //Devuelve el tipo de dato de la variable, en este caso number.
const entero =  new Number(10); 
const palabrota = new String("Holasa");  //Usar estos constructores no es recomendable, de hecho es lo mismo si declaro una variable como un numero o un string literal sin ser new Number o new String, ya que el tipo de dato es el mismo.
const AltoNumero = 99999999999999999n; //Numero por fuera del rango de los numeros normales
typeof AltoNumero; //Devuelve el tipo de dato de la variable, en este caso BigInt.
...
const simbolo = Symbol("hola"); //El simbolo es un tipo de dato que se usa para crear identificadores unicos y no repetibles. Se puede usar para crear propiedades de objetos que no se pueden sobrescribir.
let simoblocompartido = Symbol.for("hola"); 
...
/* for (iterador; condicion; iteracion) {
  //El iterador es el valor inicial que indica como va iterando el loop, la condicion es el valor que se tiene que cumplir para que el loop siga y la iteracion es lo que se le suma al iterador cada vez que se cumple la condicion.
  logica;
} */
for (let i = 0; i < contador; i = i + 1) {
  // i = i + 1 es igual a i +=1 que es lo mismo a i++ tambien se pueden usar los -- o i-=1 para la resta
...
function nombre(parametro, parametro2) {
  "use strict"; //Sirve para usar el modo estricto de javascript, que es un modo que se usa para evitar errores comunes en el codigo. Por ejemplo, si se intenta usar una variable sin declararla, el programa no va a funcionar y dara error. El modo estricto es una buena practica para evitar errores comunes en el codigo.
  //La funcion es un bloque de codigo que se utiliza para reutilizar en la aplicacion.
...
while (condicion) {
  //Los ciclos while son aquellos que se repiten hasta que la condicion se cumpla.
  logica;
}
...
function piramide(filas, columnas) {
  return (
    " ".repeat(columnas - filas) +
    hash.repeat(2 * filas - 1) +
    " ".repeat(columnas - filas)
  ); //El repeat es un metodo que se usa para repetir una variable las veces que se indica. En este caso, el hash se va a repetir i veces, donde i es el iterador que va desde 1 hasta 10. Por lo que la piramide va a ir creciendo de a 1 cada vez que se cumple la condicion.
}
